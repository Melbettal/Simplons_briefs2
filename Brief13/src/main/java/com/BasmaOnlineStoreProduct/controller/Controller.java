	...
	public ResponseEntity<Object> saveCategory(@RequestBody Categorie categorie) throws IOException {
		try {
			...
			if (image != null) {
				...
				return new ResponseEntity<>("ajout terminer avec succes", HttpStatus.OK);
			} else {
	...
	public ResponseEntity<Object> saveProduct(@RequestBody Produit produit) throws IOException {
		...
		if (produit.getImages().size() >= 4 && produit.getImages().size() <= 8) {
			...
			if(categorieRepository.existsById(produit.getCategorie().getId()) == true) {
				...
				return new ResponseEntity<>("id categorie est invalide", HttpStatus.NOT_FOUND);
			}
		} else {
			return new ResponseEntity<>("nombre d'image doit etre entre 4 et 8", HttpStatus.LENGTH_REQUIRED);
		}
		}else {
			return new ResponseEntity<>("le nom de produit et deja exist", HttpStatus.NOT_FOUND);
		}
